...
export function canPlace(
  ...
  y: number,
  ignore?: string,
  tiles: TerrainTile[] = terrain,
) {
  const d = getBuilding(type);
  if (x < 0 || y < 0 || x + d.w > MAP_SIZE || y + d.h > MAP_SIZE) return false;
  const cells = occupied(buildings, ignore);
  for (let xx = x; xx < x + d.w; xx++)
    for (let yy = y; yy < y + d.h; yy++) {
      const tile = terrainAt(tiles, xx, yy);
      if (cells.has(`${xx},${yy}`) || !tile || !isBuildableTerrain(tile.type))
        return false;
    }
  return true;
}
